...
//         className="w-full flex justify-between items-center p-4 bg-gray-100 dark:bg-background-dark hover:bg-gray-200 dark:hover:bg-gray-700 rounded-t-lg"
//       >
...
const ProjectDetailsResidentialStep: React.FC<ProjectDetailsResidentialStepProps> = ({
  ...
  // Preset land sizes (from search: common Kenyan plots)
    ...
    <section className="max-w-3xl mx-auto">
      ...
      <AccordionSection title="Land Details" id="land">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              value={landSizePresets.find(p => p.sqm === localData.land?.size)?.label || ""}
              onChange={(e) => {
                const selected = landSizePresets.find(p => p.label === e.target.value);
                if (selected) handleChange("land.size", selected.sqm);
              }}
              className="w-full border rounded-lg text-xs md:text-sm p-2 bg-white dark:bg-background-dark mb-4"
            >
              <option value="">Select Size</option>
              {landSizePresets.map(p => <option key={p.label} value={p.label}>{p.label}</option>)}
            </select>
            {localData.land?.size === 0 && (  // Custom input if "Other"
              <input
                type="number"
                value={localData.land?.size || ""}
                onChange={(e) => handleChange("land.size", Number(e.target.value))}
      ...
      <AccordionSection title="Perimeter Wall" id="perimeterWall">
        <div className="space-y-2">
          ...
          {/* ... existing include checkbox */}
          {localData.perimeterWall?.include && (
            <div>
              <div className="space-y-2">
          ...
          {localData.sewage?.type === 'septic' && (
            <div className="flex items-center space-x-2">
              ...
              <input 
                ...
                onChange={(e) => handleChange("sewage.septicSize", e.target.value)} 
                className="w-full border rounded-lg p-2 text-xs md:text-base dark:bg-background-dark dark:border-gray-700" 
                placeholder="e.g., 500"  
              />
